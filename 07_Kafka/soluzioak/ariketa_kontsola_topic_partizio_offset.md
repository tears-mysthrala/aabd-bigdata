...
Iturria: [Apache Kafka 3.7 PDFa](../materialak/01_03_ApacheKafka.pdf), 23–27. orrialdeak (ariketak 1–5). Komandoak `infra/docker-compose.yml`-ko `apache/kafka:3.7.0` irudi eta `iabd-kafka` container-erako berrikusi dira. Azalpen hau **komandoak berrikusiak** egoeran dago; ez dira brokerrean exekutatu. Ez da zerbitzurik abiarazi/gelditu, topic-ik sortu edo aldatu, ezta bolumena ukitu ere.

## Segurtasunez nola erabili

Ikasgelako enuntziatuek `sentsoreak` eta `erosketak` izenak eskatzen dituzte. Izen horiek agindu didaktikoak dira: erabili zure ariketarako izendatutako broker batean, eta ez partekatutako `iabd-kafka` brokerrean. Repositorio honek broker eta `iabd-kafka-data` bolumen iraunkorra partekatzen ditu (`infra/`); beraz, gida honek zuzeneko adibide guztiak `codex-aabd-20260924-` aurrizkiarekin izendatzen ditu. Aginduek topic-ak sortu edo aldatuko lituzkete exekutatuz gero; hemen **ez dira exekutatu**.

Ingurune honetako CLI bidea `/opt/kafka/bin/` da eta broker helbidea `localhost:9092`. Ez abiarazi Compose zerbitzurik gida hau erabiltzeko: dagokion inguruneak dagoeneko martxan izan behar du eta ariketa broker horretan egiteko baimena eman behar zaizu. Adibideek ezin dituzte aurreikusi tokiko topic-zerrenda, leaderrak, mezuen banaketa edo benetako offset-ak.

Komandoetan izen prefijatuak eta `localhost:9092` helbidea zuzenean idatzita daude, kopiatzean shell-aldagairik edo topic izen orokorrik nahas ez dadin. Agindu guztiak berrikusiak dira soilik. Ez exekutatu labekoak ez diren broker/topic/bolumen batean.
...
```bash
docker exec iabd-kafka /opt/kafka/bin/kafka-topics.sh --create --topic codex-aabd-20260924-sensors --partitions 2 --bootstrap-server localhost:9092
docker exec iabd-kafka /opt/kafka/bin/kafka-topics.sh --create --topic codex-aabd-20260924-purchases --partitions 4 --bootstrap-server localhost:9092
docker exec iabd-kafka /opt/kafka/bin/kafka-topics.sh --list --bootstrap-server localhost:9092
docker exec iabd-kafka /opt/kafka/bin/kafka-topics.sh --describe --topic codex-aabd-20260924-sensors --bootstrap-server localhost:9092
docker exec iabd-kafka /opt/kafka/bin/kafka-topics.sh --describe --topic codex-aabd-20260924-purchases --bootstrap-server localhost:9092
```
...
|---|---:|---|
| `codex-aabd-20260924-sensors` | 2 | `0`, `1` |
| `codex-aabd-20260924-purchases` | 4 | `0`, `1`, `2`, `3` |
...
```bash
docker exec -it iabd-kafka /opt/kafka/bin/kafka-console-producer.sh --topic codex-aabd-20260924-sensors --bootstrap-server localhost:9092
```
...
```bash
# Lehen consumer-a
docker exec -it iabd-kafka /opt/kafka/bin/kafka-console-consumer.sh --topic codex-aabd-20260924-sensors --group codex-aabd-20260924-sensor-read-1 --from-beginning --bootstrap-server localhost:9092
```
...
```bash
docker exec -it iabd-kafka /opt/kafka/bin/kafka-console-consumer.sh --topic codex-aabd-20260924-sensors --group codex-aabd-20260924-sensor-read-2 --from-beginning --bootstrap-server localhost:9092
```
...
```bash
docker exec -it iabd-kafka /opt/kafka/bin/kafka-console-producer.sh --topic codex-aabd-20260924-purchases --bootstrap-server localhost:9092
```
...
```bash
docker exec -it iabd-kafka /opt/kafka/bin/kafka-console-consumer.sh --topic codex-aabd-20260924-purchases --group codex-aabd-20260924-purchases-read-1 --from-beginning --bootstrap-server localhost:9092
```

Itxi consumer-a `Ctrl-C` bidez eta ireki beste bat, group ID berriarekin:

```bash
docker exec -it iabd-kafka /opt/kafka/bin/kafka-console-consumer.sh --topic codex-aabd-20260924-purchases --group codex-aabd-20260924-purchases-read-2 --from-beginning --bootstrap-server localhost:9092
```
...
```bash
docker exec -it iabd-kafka /opt/kafka/bin/kafka-console-consumer.sh --topic codex-aabd-20260924-purchases --group codex-aabd-20260924-purchases-offsets --from-beginning --property print.partition=true --property print.offset=true --bootstrap-server localhost:9092
```
...
```bash
docker exec iabd-kafka /opt/kafka/bin/kafka-topics.sh --describe --topic codex-aabd-20260924-sensors --bootstrap-server localhost:9092
docker exec iabd-kafka /opt/kafka/bin/kafka-topics.sh --alter --topic codex-aabd-20260924-sensors --partitions 4 --bootstrap-server localhost:9092
docker exec iabd-kafka /opt/kafka/bin/kafka-topics.sh --describe --topic codex-aabd-20260924-sensors --bootstrap-server localhost:9092
```
...
```bash
docker exec iabd-kafka /opt/kafka/bin/kafka-topics.sh --alter --topic codex-aabd-20260924-sensors --partitions 2 --bootstrap-server localhost:9092
```
...
| Compose zerbitzua, container izena, CLI bidea, Kafka bertsioa eta volume izena | `infra/docker-compose.yml`-tik berrikusia |
