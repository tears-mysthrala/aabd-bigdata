...
def run_kafka(n: int, topic: str, bootstrap: str, keys: bool) -> None:
    ...
    try:
        for i in range(n):
            ...
            key = f"gakoa{i % 2}".encode("utf-8") if keys else None
...
def main() -> None:
    ...
    if args.mock:
        ...
        run_kafka(args.n, args.topic, args.bootstrap, args.keys)
        print(f"kafka: {args.n} mezu -> {args.topic}")
